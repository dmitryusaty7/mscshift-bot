...
async function handleHoldsInput({ bot, chatId, telegramId, text, shiftsRepo, holdsRepo, messages, logger }) {
  const session = shiftSessions.get(telegramId)
  ...
  try {
    ...
    await holdsRepo.createForShift({ shiftId: shift.id, count: holdsCount })
